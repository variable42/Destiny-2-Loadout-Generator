...
val joined = helmets
    ...
    //   .withColumn("Combo_ResRec", floor($"Resilience" / 10) + floor($"Recovery" / 10))
      .withColumn("Combo_MobRec", floor($"Mobility" / 10) + floor($"Recovery" / 10))
    ...
    .withColumn("Combo_MobRec_MW", floor($"Mobility_MW" / 10) + floor($"Recovery_MW" / 10))
    //   .withColumn("Combo_ResRec_MW", floor($"Resilience_MW" / 10) + floor($"Recovery_MW" / 10))
    .withColumn("Discipline_MW", $"Discipline_MW_H" + $"Discipline_MW_G" + $"Discipline_MW_C" + $"Discipline_MW_L" + $"Discipline_MW_CL")
    //   .withColumn("Combo_RecDis_MW", floor($"Recovery_MW" / 10) + floor($"Discipline_MW" / 10))
